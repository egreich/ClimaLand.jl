...
export SatisficingConductanceParameters, SatisficingConductance
...
"""
    SatisficingConductance{FT}(;
        Drel::FT = FT(1.6),              # H2O/CO2 diffusivity ratio (unitless)
        safety_margin::FT = FT(0.20),    # (Ecrit - E)/Ecrit minimum
        min_photosynthesis_ratio::FT = FT(0.70),  # A >= this × max(A)
        max_hydraulic_risk::FT = FT(0.80),        # 1 - K(Pleaf)/Kmax <= this
        frac_kcrit::FT = FT(0.05),       # K(Pcrit) = frac_kcrit * Kmax
        selection::Symbol = :minE        # :minE | :maxA | :median
    ) where {FT <: AbstractFloat}

Creates a SatisficingConductance using default parameters of type `FT`.
"""
# Convenience outer ctor with keywords (accepts Symbol/Int `selection` OR `selection_code`)
function SatisficingConductance{FT}(;
    Drel::FT,
    g0::FT = zero(FT),  # compatibility; ignored by satisficing
    g1::FT = zero(FT),  # compatibility; ignored by satisficing
    ...
    params = SatisficingConductanceParameters{FT}(;
        Drel,
        g0,
        g1,
        safety_margin,
    ...
    return SatisficingConductance{FT, typeof(params)}(params)
end
